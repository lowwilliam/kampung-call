...
try {
  ...
  const chrome = path.join('/Applications', 'Google Chrome.app', 'Contents', 'MacOS', 'Google Chrome');
  if (!fs.existsSync(chrome)) throw new Error('Google Chrome is required for the headless visibility check');
  await new Promise((resolve, reject) => {
    const check = spawn(process.execPath, [path.join(root, 'review-shots', 'shoot-cdp.cjs')], {
      cwd: root,
      env: {...process.env, GAME_URL: 'http://127.0.0.1:5199/', VERIFY_ONLY: '1'},
      stdio: 'inherit',
    });
