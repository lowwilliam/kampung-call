...
const originalAssetIds = new Set(GAME_ASSETS.map((asset) => asset.id));

async function isPublishedAsset(assetId: string) {
  if (originalAssetIds.has(assetId)) return true;
  const { DB } = bindings();
  const published = await DB.prepare("SELECT id FROM submissions WHERE id = ? AND status = 'published'")
    .bind(assetId)
    .first<{ id: string }>();
...
export async function POST(request: Request) {
  ...
  if (!assetId || assetId.length > 100 || !(await isPublishedAsset(assetId))) {
