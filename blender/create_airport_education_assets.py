...
def build_nus_sde4():
    ...
    for x in (-2.45, -1.48, -.50, .50, 1.48, 2.45):
        cube("Facade mullion", (x, -1.03, 2.42), (.055, .10, 2.44), TEAL, root, edge=.008)
    cube("Deep overhanging roof", (0, .20, 4.02), (7.65, 3.75, .28), CHALK, root, edge=.12)
    cube("Vertical end fin", (-3.50, .18, 2.42), (.28, 3.55, 3.32), CHALK, root, edge=.065)
    for index in range(6):
        cube("External stair", (-3.18 + index*.34, -1.78, .34 + index*.18),
             (.42, .72, .12), CHALK, root, edge=.025)
    for x in (-2.15, 0, 2.15):
        cube("Terrace planter", (x, -1.68, 1.35), (1.08, .38, .27), GREEN2, root, edge=.05)
        for px in (-.30, 0, .30):
            ico("Terrace planting", (x+px, -1.68, 1.62), .25, GREEN2, root,
                scale=(1.1, .65, .72))
...
def build_ntu_hive():
    ...
    for tower_index in range(12):
        ...
        tower.parent = root
        cyl("Warm vertical core", (x, y, 1.72), .25, 3.05, HIVE_CONCRETE, tower, 14)
        ...
        torus("Tower crown rib", (x, y, 3.08), .58, .060, HIVE_CONCRETE, tower, major_segments=20)
    cube("NTU marker", (-2.75, -2.25, .46), (1.35, .20, .62), HIVE_CONCRETE, root, edge=.065)
    label("NTU", (-2.75, -2.37, .50), .25, INK, root)
    return root
...
def build_smu_law():
    ...
    for x in (-3.30, -2.38, -1.46, -.54, .38):
        cube("Law School fin", (x, -1.05, 1.86), (.09, .18, 2.76), TEAL, root, edge=.012)
    ...
    for rib_index in range(-3, 4):
        ...
        cable("Pearl shell rib", [(2.45+x*.72, -.66, .70), (2.45+x, -1.04, 1.46),
              (2.45+x*.72, -.66, 2.32)], .035, CHALK, root)
    for z, radius in ((1.00, 1.13), (1.49, 1.43), (1.98, 1.12)):
        torus("Pearl horizontal band", (2.45, -.05, z), radius, .035, CHALK, root,
              major_segments=28)
    for index in range(5):
        cube("Public terrace step", (1.35+index*.22, -1.70, .18+index*.10),
             (3.25-index*.30, .62, .10), CHALK, root, edge=.025)
    for x in (-3.65, 3.65):
        cyl("Campus tree trunk", (x, 1.68, .48), .08, .82, TEAL, root, 12)
        ico("Campus tree crown", (x, 1.68, 1.15), .55, TEAL, root, scale=(1, .82, 1.05))
    cube("SMU marker", (-3.15, -2.05, .48), (1.25, .20, .62), TEAL, root, edge=.065)
...
def build_sutd_campus():
    ...
    cube("Open central court", (0, .18, .18), (3.8, 2.25, .14), GREEN2, root, edge=.12)
    for wing_index, y in enumerate((-1.48, 1.62)):
        ...
        cube("Planted roof insert", ((-1.65 if wing_index == 0 else 1.65), y, 3.14),
             (2.25, .88, .24), GREEN2, root, edge=.10)
    cube("Learning spine bridge", (0, .08, 1.95), (1.60, 3.72, .48), SUTD_WHITE, root, edge=.14)
    ...
    cube("SUTD marker", (3.32, -2.28, .48), (1.40, .20, .62), TEAL, root, edge=.065)
    label("SUTD", (3.32, -2.40, .51), .23, SUTD_WHITE, root)
    return root
